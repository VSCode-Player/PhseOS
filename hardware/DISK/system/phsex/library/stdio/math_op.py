...
def rem(first_addr, second_addr, result_addr=None):
    ...
    if not FIRST_ADDR["file"] == "TYPE:INT":
        ...
        if not SECOND_ADDR["file"] in ["TYPE:INT","TYPE:STRING"]:
            ...
            op_stop_os("Error from REM:Second addres cannot be INT or STRING.",1,error_args=f"{first_addr} {second_addr}")
    else:
        result = 0
        op_stop_os("Error from REM:First addres cannot be STRING.",1,error_args=f"{first_addr} {second_addr}")
