...
def op_stop_os(stop_message:str, exit_code:int,error_args="",):
    from phsex import process_list,symbol_table,lable_table,imported_package
...
def op_data_transform(data, mode, CharSet="PhseEncode.json"):
    if mode == CHAR_TO_BIN:
        ...
        return f"{int(float(data)) & 0xFF:08b}"
